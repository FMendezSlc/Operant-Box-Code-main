...
from com.data_logger_old import Data_logger
...
def task_menu(board, task):
    ...
    subject_ID = None
    while True:
        i = input('\nPress [enter] to run task, [g] to get variable value, [s] to set variable'
                  ' value, [c] to close program, [f] to create data file, or [t] to select a new '
                  'task:')
        if i == '':
            if subject_ID: 
